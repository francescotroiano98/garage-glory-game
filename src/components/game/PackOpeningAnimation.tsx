...
type Phase = 'shaking' | 'ripping' | 'reveal' | 'summary';
...
export function PackOpeningAnimation({ cards, packIcon = '📦', onClose }: Props) {
  const { t } = useLanguage();
  ...
  return (
    ...
    >
      {/* Phase 1: pack shakes */}
      {phase === 'shaking' && (
        <div className="text-[120px] anim-pack-shake drop-shadow-[0_0_30px_hsl(var(--primary)/0.6)]">
          {packIcon}
        </div>
      )}
